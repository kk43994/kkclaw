...
const ThemeManager = require('./theme-manager'); // 🎨 主题管理
...
async function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    y: petConfig.get('position')?.y || height - 260,
    frame: false,
    transparent: true,
  ...
  // 🎨 初始化主题管理器
  themeManager = new ThemeManager(petConfig);
  ...
  themeManager.on('theme-changed', (theme) => {
    ...
    if (voiceSystem) {
      voiceSystem.speak(`已切换到${theme.name}主题`, { priority: 'normal' });
    }
  });

  mainWindow.loadFile('index-themed.html');
...
// 🎨 主题切换时调整窗口大小
ipcMain.on('theme-resize-window', (event, { width: w, height: h }) => {
  if (!mainWindow) return;
  // 保持原始 67px 流体球对应 200x260 的窗口基线
  const petW = w || 67;
  const petH = h || 67;
  const winW = Math.max(200, Math.min(460, petW + 133));
  const winH = Math.max(260, Math.min(560, petH + 193));
  mainWindow.setSize(winW, winH);
  console.log(`🎨 窗口调整: ${winW}x${winH} (宠物 ${w}x${h})`);
});
...
ipcMain.handle('theme-switch', async (event, themeId) => {
  if (!themeManager) return { success: false, error: 'ThemeManager not initialized' };
  try {
    const theme = await themeManager.switchTheme(themeId);
...
ipcMain.handle('theme-import', async (event, filePath) => {
  if (!themeManager) return { success: false, error: 'ThemeManager not initialized' };
  try {
    // 如果没传路径，弹出系统文件选择对话框
    ...
    if (!targetPath) {
      ...
      const result = await dialog.showOpenDialog(win, {
        ...
        filters: [
          { name: '主题文件', extensions: ['json', 'kktheme', 'zip'] },
    ...
    const importResult = await themeManager.importTheme(targetPath);
...
ipcMain.handle('theme-export', async (event, themeId, destPath) => {
  if (!themeManager) return { success: false, error: 'ThemeManager not initialized' };
  try {
    ...
    if (!targetPath) {
      ...
      if (result.canceled || !result.filePaths?.length) return { success: false, error: 'cancelled' };
      targetPath = path.join(result.filePaths[0], theme?.id || themeId);
    }
    return await themeManager.exportTheme(themeId, targetPath);
  } catch (err) {
    return { success: false, error: err.message };
  }
});

ipcMain.handle('theme-export-package', async (event, themeData, files, destPath) => {
  try {
    if (!themeData?.id) return { success: false, error: '缺少主题 id' };
    ...
    if (!targetPath) {
      ...
      if (result.canceled || !result.filePaths?.length) return { success: false, error: 'cancelled' };
      targetPath = path.join(result.filePaths[0], themeData.id);
    }
    ...
    if (files && Array.isArray(files)) {
      for (const f of files) {
        ...
        if (!match) continue;
        const filePath = path.join(finalPath, f.name);
        await fsp.mkdir(path.dirname(filePath), { recursive: true });
        await fsp.writeFile(filePath, Buffer.from(match[1], 'base64'));
      }
    }

    await fsp.writeFile(path.join(finalPath, 'theme.json'), JSON.stringify(themeData, null, 2));
...
ipcMain.handle('theme-open-picker', async () => {
  if (themePickerWindow && !themePickerWindow.isDestroyed()) {
    themePickerWindow.focus();
    return;
  }
  themePickerWindow = new BrowserWindow({
    width: 480,
    height: 520,
    frame: true,
    resizable: true,
  ...
  themePickerWindow.loadFile('theme-picker.html');
  themePickerWindow.on('closed', () => { themePickerWindow = null; });
});

// 🎨 主题编辑器
let themeEditorWindow = null;

ipcMain.handle('theme-open-editor', async () => {
...
ipcMain.handle('theme-save-and-apply', async (event, themeData, files) => {
  if (!themeManager) return { success: false, error: 'ThemeManager not initialized' };
  try {
    // 保存到 skins/user/<id>/theme.json
    const destDir = path.join(__dirname, 'skins', 'user', themeData.id);
    const fsp = require('fs').promises;
    await fsp.mkdir(destDir, { recursive: true });
    // 保存上传的图片文件（base64 dataUrl → 二进制）
    if (files && Array.isArray(files)) {
      for (const f of files) {
        if (!f.name || !f.dataUrl) continue;
        const match = f.dataUrl.match(/^data:[^;]+;base64,(.+)$/);
        if (match) {
          await fsp.writeFile(path.join(destDir, f.name), Buffer.from(match[1], 'base64'));
        }
      }
    }
    await fsp.writeFile(path.join(destDir, 'theme.json'), JSON.stringify(themeData, null, 2));
    // 重新扫描并切换
    await themeManager.reload();
    const theme = await themeManager.switchTheme(themeData.id);
    return { success: true, theme };
  } catch (err) {
    return { success: false, error: err.message };
  }
...
ipcMain.handle('theme-export-json', async (event, jsonString, destPath) => {
  try {
    const fsp = require('fs').promises;
    ...
    resolved = resolved.replace(/^~/, require('os').homedir());
    await fsp.writeFile(resolved, jsonString);
...
ipcMain.handle('theme-ai-generate', async (event, userPrompt) => {
  if (!openclawClient) return { success: false, error: 'OpenClaw 未初始化' };
  try {
    const systemPrompt = `你是 KKClaw 桌宠主题创作助手。用户会描述他们想要的宠物外观，你需要生成一个完整的 theme.json 配置。

主题类型为 css-fluid（流体球），通过 CSS 渐变色和眼睛表情表达情绪。
...
- description: 描述
- type: "css-fluid"
- size: {width, height} (建议 60-80)
- shape: {borderRadius: "50%" 或其他, padding: 3}
- eyes: {count:2, gap:"12px", default:{w,h,br}, color:"颜色", glow:"发光"}
- decorations: {blush:{enabled:bool}, particles:{colors:["颜色数组"]}}
- moods: 14种情绪(offline,idle,happy,talking,thinking,sleepy,surprised,sad,angry,fearful,calm,excited,love,focused)，每种包含:
  - fluid: CSS linear-gradient
  - b1, b2: CSS radial-gradient (用 rgba)
  - glow: CSS box-shadow
  - eyes: 表情名(normal/happy/sad/thinking/surprised/sleepy/angry/love/nervous/talking/superHappy/focus/softSmile)
  - scale: 缩放(0.9-1.1)
  - bounce: 弹跳(0-0.03)
  - speed: 流体速度("3s"-"20s")
- animation: {float,rotation,breath,fidget,deepBreath} 参数

注意:
1. 配色要和谐美观，每种情绪色调不同但风格统一
2. 只输出 JSON，不要其他文字
3. 情绪配色要有区分度：happy用暖色，sad用冷色，angry用红色系等`;

    const fullPrompt = `${systemPrompt}\n\n用户要求: ${userPrompt}`;
    const response = await openclawClient.sendMessage(fullPrompt);

    if (!response || response.startsWith('请求失败') || response.startsWith('连接失败')) {
      return { success: false, error: response || 'AI 无响应' };
    }

    // 尝试提取 JSON
    let jsonStr = response;
    // 如果 AI 返回了 markdown 代码块，提取内容
    ...
    return { success: true, theme, raw: response };
  } catch (err) {
    return { success: false, error: err.message };
  }
});
